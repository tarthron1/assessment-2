...
function canWeDeliverTwo(zipCode){
    let responce = "Sorry, we can't deliver to that address."
    for ( i = 0; i <= deliveryAreaZipCodes.length; i++){
        if (deliveryAreaZipCodes[i] === zipCode){
            responce = "You're in our delivery zone!"
            return responce
        }
    }
    return responce
...
deals[1] = deals[1].replace("March","April")
deals[1] = deals[1].trim()